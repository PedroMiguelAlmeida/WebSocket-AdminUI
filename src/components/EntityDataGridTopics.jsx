...
const EntityDataGridTopics = ({
  ...
  if (
    data &&
    (topicData.namespace !== location.state.namespace || topicData.length == 0)
  ) {
    setTopicData(data);
  }
    ...
    if (lastJsonMessage !== null) {
      ...
      setMessageHistory((prev) => {
        const alreadyExists = !!prev.filter(
          (msg) => msg.payload.msgDate === lastJsonMessage.payload.msgDate
        ).length;
        if (!alreadyExists) {
  ...
  const deleteTopic = async (topic) => {
    ...
    await fetch(
      `http://localhost:3001/api/namespaces/${location.state.namespace}/topics/${topic}`,
      {
        method: "DELETE",
  ...
  const handleDelete = async (arrTopics) => {
    ...
    for (let i = 0; i < arrTopics.length; i++) {
      console.log(` Handle Delete Topic ${arrTopics[i].toString()}`);
      ...
      const topicIndex = namespace.topics.findIndex(
        (topic) => topic.topicName === arrTopics[i].toString()
      );
  ...
  const handleRefresh = async (topicData) => {
    const refreshedTopics = await fetch(
      `http://localhost:3001/api/namespaces/${location.state.namespace}`,
      {
        method: "GET",
	...
	const topicRefreshed = await refreshedTopics.json();
    if (topicRefreshed) {
      setTimeout(() => {
        setTopicData({ ...topicData, topics: topicRefreshed.topics, clients: topicRefreshed.clients });
      }, 1000);
      console.log("Refreshed Topics", topicRefreshed);
    }
  };
  ...
  const broadcast = async (values, onSubmitProps) => {
    const broadcastResponse = await fetch(
      `http://localhost:3001/api/namespaces/${location.state.namespace}/broadcast`,
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
