...
const EntityDataGridNamespaces = ({
  ...
  const handleDelete = async (arrNamespaces) => {
    for (let i = 0; i < arrNamespaces.length; i++) {
      ...
      setNamespaceData(
        data.filter((d) => d.namespace !== arrNamespaces[i].toString())
      );
    }
  };

  const handleRefresh = async () => {
    const refreshedNamespaces = await fetch(
      `http://localhost:3001/api/namespaces`,
      {
        method: "GET",
	...
	const namespacesRefreshed = await refreshedNamespaces.json();
    if (namespacesRefreshed) {
      setTimeout(() => {
        setNamespaceData(namespacesRefreshed);
      }, 1000);
      console.log("Refreshed Namespaces", namespacesRefreshed);
    }
  };
