...
const CreateTopicForm = ({setTopicData,topicData}) => {
	...
	return (
		<Formik onSubmit={handleFormSubmit} initialValues={defaultValuesTopic} validationSchema={topicSchema}>
			{({ values, errors, touched, handleBlur, handleChange, handleSubmit, setFieldValue, resetForm }) => (
				<form onSubmit={handleSubmit}>
					<Box display="grid" gap="25px">
						...
						/>
						<Dropzone acceptedFiles=".json, .json.schema" multiple={false} onDrop={(acceptedFiles) => (setFieldValue = ("schema", acceptedFiles[0]))}>
							{({ getRootProps, getInputProps }) => (
								<Box {...getRootProps()} border="2px" p="1rem" sx={{ "&:hover": { cursor: "pointer" } }}>
									<input {...getInputProps()} />
									{!values.schema ? (
