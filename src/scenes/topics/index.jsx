...
import { Box, Card, CardActions, CardContent, Collapse, Button, useMediaQuery, Typography, useTheme } from "@mui/material";
	...
	return (
		...
		>
			<CardContent>
				...
				<Typography sx={{ mb: "0.5rem" }}>Namespace: {data.namespace}</Typography>
			</CardContent>
			<CardActions>
			...
			{data || !isLoading ? (
				...
				>
					{data.topics.map((entry) => (
						<Topic key={entry._id} topicName={entry.topicName} namespace={data.namespace} />
					))}
				</Box>
			) : (
				<>Topics data loading...</>
